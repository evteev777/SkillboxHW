...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        try (SessionFactory sessionFactory = metadata.getSessionFactoryBuilder().build();
            ...
            totalMapping(COURSE_ID_3 ,dateFormat, session, appLogger);
        } catch (Exception e) {
            appLogger.error(e.getStackTrace());
        }
    ...
    private static void subscriptionMapping(int COURSE_ID, int SUBSCRIPTION_NUMBER,
        ...
        Course course = session.get(Course.class, COURSE_ID);
        Subscription.Key key = course.getSubscriptions().get(SUBSCRIPTION_NUMBER).getKey();
